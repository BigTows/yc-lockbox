...
const yandex_lockbox_repository_1 = __importDefault(__nccwpck_require__(9483));
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const iamToken = core.getInput('iam-token');
            const secretId = core.getInput('secret-id');
            const versionId = core.getInput('version-id').trim().length === 0
                ? undefined
...
const YC_LOCKBOX_API = 'https://payload.lockbox.api.cloud.yandex.net/lockbox/v1/secrets/%_SECRET_ID_%/payload';
class YandexLockboxRepository {
    constructor(iamToken, secretId, versionId) {
        this.storage = [];
        this.yandexLockboxPayloadUrl = YC_LOCKBOX_API.replace('%_SECRET_ID_%', secretId);
        this.axiosConfiguration = YandexLockboxRepository.buildAxiosParams(iamToken, versionId);
    }
    /**
    ...
    load() {
        return __awaiter(this, void 0, void 0, function* () {
            const response = yield axios_1.default.get(this.yandexLockboxPayloadUrl, this.axiosConfiguration);
            if (response.status !== 200) {
                throw new Error(`Can't load secrets for Lockbox, status: ${response.status} (${response.statusText})`);
            }
    ...
    findOne(secretKey) {
        const result = this.storage.find((keyValue) => {
...
exports.default = YandexLockboxRepository;
